...
<div class="w-full bg-white rounded-xl shadow-sm border p-4 sm:p-6 md:p-8">
    ...
    <div class="mb-6 sm:mb-8 border-b pb-4">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Tambah Siswa Baru</h1>
        <p class="text-xs sm:text-sm text-gray-500 mt-1">Isi formulir di bawah ini dengan data siswa yang valid.</p>
    </div>

    <form action="{{ route('admin.siswa.store') }}" method="POST" class="space-y-6 sm:space-y-8">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 bg-gray-50/50 p-4 sm:p-6 rounded-xl border">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="alamat" id="alamat" rows="3" placeholder="Masukkan alamat domisili siswa"
                        class="w-full px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-lg border focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-xs sm:text-sm @error('alamat') border-red-500 @enderror">{{ old('alamat') }}</textarea>
                    @error('alamat')
                    <p class="text-xs text-red-500 mt-1 flex items-center gap-1">
                        <i class="fa-solid fa-circle-exclamation"></i>{{ $message }}
                    </p>
